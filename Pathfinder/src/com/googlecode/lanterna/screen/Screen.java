...
/**
 * A layer to put on top of a Terminal object, giving you a kind of screen
 * buffer to use, which is a lot easier to work with. Drawing text or graphics
 * to the terminal is kind of like writing to a bitmap.
 * 
 * @author Martin
 */
public class Screen
{
    ...
    /**
     * Creates a new Screen on top of a supplied terminal, will query the
     * terminal for its size. The screen is initially blank.
     * 
     * @param terminal
     ...
     *            Height (number of rows) of the terminal
     */
    ...
    /**
     * Moves the current cursor position or hides it. If the cursor is hidden
     * and given a new position, it will be visible after this method call.
     * 
     * @param position
     *            0-indexed column and row numbers of the new position, or if
    ...
    /**
     * Moves the current cursor position, and if the cursor was hidden it will
     * be visible after this call
     * 
     * @param column
     *            0-indexed column number of the new position
     * @param row
     *            0-indexed row number of the new position
     */
    ...
    public void stopScreen()
    {
        ...
        {
            // Drain the input queue before exiting private mode and closing the
            // Screen.
        }
     ...
     * 
     * @param x
     *            0-indexed column number of where to put the first character in
     *            the string
     * @param y
     *            0-indexed row number of where to put the first character in
     *            the string
     * @param string
    ...
    /**
     * Draws a character on the screen at a particular position
     * 
     * @param x
     *            0-indexed column number of where to put the character
     * @param y
     *            0-indexed row number of where to put the character
     * @param character
     *            Text to put on the screen
     * @param foregroundColor
     *            What color to use for the text
     ...
     *            Additional styles to apply to the text
     */
    public void putC(int x, int y, char character, Terminal.Color foregroundColor, Terminal.Color backgroundColor, ScreenCharacterStyle... styles)
    {
        Set<ScreenCharacterStyle> drawStyle = EnumSet.noneOf(ScreenCharacterStyle.class);
        drawStyle.addAll(Arrays.asList(styles));
        putCharacter(x, y, new ScreenCharacter(character, foregroundColor, backgroundColor, drawStyle));
    }
     ...
     * 
     * @return true if the size is the same as before, false if the screen
     *         should be resized
     */
    ...
    /**
     * Calling this method will check if the terminal has changed since and in
     * that case update the dimensions of this Screen to match.
     * 
     * @return Will return true if dimensions were changed, otherwise false. You
     *         probably want to clear and redraw the entire screen if this
     *         method returns true.
     */
     ...
     * This is useful of something has written to the terminal outside of the
     * Screen (System.out or through direct calls to the underlying Terminal)
     * and you want to make sure that the content of Screen is completely pushed
     * to the terminal.
     */
